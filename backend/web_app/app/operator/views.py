...
import datetime
from app.views import executeQuery,isOperator,userToPath
def myconverter(o):
...
class createUser(TemplateView):
    def get(self, request, **kwargs):
        ...
        executeQuery('insert into user_privi values(%d,3)'%(user.id),False)
...
class getListSensor(TemplateView):
    def get(self, request, **kwargs):
        if check_user(request):
            return check_user(request)
        sql='select * from sensor where id_user in ( select id_sub from user_created_by where id_super=%d)'%(int(request.user.id))
        return HttpResponse(dumps(executeQuery(sql),default=myconverter),content_type='application/json; charset=utf-8')
class getListSensorOf(TemplateView):
    def get(self, request, **kwargs):
        if check_user(request):
            return check_user(request)
        id=int(request.GET.get('id',False))
        sql='select * from sensor where id_user = %d'%(id)
        return HttpResponse(dumps(executeQuery(sql),default=myconverter),content_type='application/json; charset=utf-8')
